...
describe("buildNewsFormData", () => {
  it("builds FormData with title, summary, content and draft flag", () => {
    const fd = buildNewsFormData({ title: "T", summary: "S", content: "C", draft: true, publishedAt: null, coverFile: null });

    expect(fd.get("title")).toBe("T");
    expect(fd.get("summary")).toBe("S");
    expect(fd.get("content")).toBe("C");
    expect(fd.get("draft")).toBe("true");
  });

  it("sends an empty summary when none is given", () => {
    const fd = buildNewsFormData({ title: "T", summary: "", content: "C", draft: true, publishedAt: null, coverFile: null });
    expect(fd.get("summary")).toBe("");
  });

  it("omits publishedAt when null", () => {
    const fd = buildNewsFormData({ title: "T", summary: "S", content: "C", draft: false, publishedAt: null, coverFile: null });
  ...
  it("includes publishedAt as an ISO string when a Date is given", () => {
    ...
    const fd = buildNewsFormData({ title: "T", summary: "S", content: "C", draft: false, publishedAt: date, coverFile: null });
  ...
  it("includes coverImage when a file is given", () => {
    ...
    const fd = buildNewsFormData({ title: "T", summary: "S", content: "C", draft: false, publishedAt: null, coverFile: file });
